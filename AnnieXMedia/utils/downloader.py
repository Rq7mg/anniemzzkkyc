...
async def yt_dlp_download(link: str, **kwargs) -> Optional[str]:
    """
    Youtube, Soundcloud ve diğerleri için ana indirme fonksiyonu.
    kwargs sayesinde 'type' ve 'title' gibi dışarıdan gelen argümanları kabul eder.
    """
    loop = asyncio.get_running_loop()
    opts = get_ytdlp_base_opts()
    
    # kwargs içerisinden gelen başlığı loglama için kullanabiliriz
    title = kwargs.get("title", "Bilinmeyen Parça")
    ...
    try:
        LOGGER.info(f"İndirme başlatılıyor: {title}")
